...
def build_model_fn_optimizer():
  ...
  def model_fn(features, labels, mode):  # pylint: disable=unused-argument
    ...
    def loss_fn():
      ...
      tf.summary.scalar(name="loss", tensor=y)
      return y * y
...
def main(_):
  ...
  model_dir = "/data/leuven/319/vsc31962/tf-distribution-strategy/output"
  estimator = tf.estimator.Estimator(
    model_fn=build_model_fn_optimizer(), config=config, model_dir=model_dir)
